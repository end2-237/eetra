...
const BLOCK_TYPES = [
  ...
  { type: 'checklist', label: 'Liste de contrôle' },
] as const

interface Props { showToast: (msg: string, type?: 'ok' | 'err' | 'default') => void }

export function EditorPanel({ showToast }: Props) {
  const { title, setTitle, subtitle, setSubtitle, ref, setRef, destination, setDestination,
    confidentiality, setConfidentiality, addBlock, pages, currentPageIndex, setPageBlocks } = useDocument()
  ...
  async function handleGenerateIntro() {
    setGenLoading(true); showToast('IA en rédaction...')
    try {
      const paragraphs = await generateIntroduction(profile.name || 'l\'entreprise', title || 'ce document')
      const currentPage = pages[currentPageIndex]
      if (currentPage) {
        const newBlocks: DocBlock[] = [...currentPage.blocks, ...paragraphs.map(p => ({ id: generateId(), type: 'text' as const, content: p }))]
        setPageBlocks(currentPage.id, newBlocks)
      }
      showToast('Introduction générée !', 'ok')
    } catch { showToast('Erreur API', 'err') } finally { setGenLoading(false) }
  }
  ...
  async function handleProfessionalize() {
    ...
    } catch { showToast('Erreur API', 'err') } finally { setProLoading(false) }
  }
      ...
      <div className="p-4 flex-1">
        ...
        <div className="flex flex-col gap-3 mb-5">
          {[
            ...
            <div key={label}>
              ...
              <input className={inp + (mono ? ' font-mono' : '')} style={inpStyle} placeholder={ph}
        ...
        <div className="rounded-xl p-4 border" style={{ background: 'var(--accentS)', borderColor: 'var(--accentS2)' }}>
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--accent)' }}>IA — Professionnaliser</span>
          </div>
          ...
          <Button variant="primary" fullWidth size="sm" disabled={genLoading} onClick={handleGenerateIntro}>
            {genLoading ? <span className="w-3 h-3 rounded-full border-2 border-white/30 border-t-white animate-spin-fast" /> : <Zap size={11} />}

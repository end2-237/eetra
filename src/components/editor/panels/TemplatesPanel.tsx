        ...
        {activeTab === 'smart' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {filteredSmart.map(tpl => (
              <div key={tpl.id} onClick={() => setSelectedTemplate(tpl.id === selectedTemplate ? null : tpl.id)}
            ...
            {filteredSmart.length === 0 && <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--text4)', padding: '20px 0' }}>Aucun résultat</p>}
          </div>
        )}
          ...
          myTemplates.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {filteredMine.map(tpl => (
                <div key={tpl.id} onClick={() => setSelectedCustomId(selectedCustomId === tpl.id ? null : tpl.id)}
                  style={{ borderRadius: 10, border: '1.5px solid', borderColor: selectedCustomId === tpl.id ? 'var(--accent)' : 'var(--border)', background: selectedCustomId === tpl.id ? 'var(--accentS)' : 'var(--surface)', cursor: 'pointer', transition: 'all .12s', overflow: 'hidden' }}>

...
import { generateSignature, buildQrUrl } from '@/lib/utils'
...
export function CoverPage({ name, tagline, signer, color, logoDataUrl, email, web, city, docId, date }: Props) {
  const { title, subtitle, ref, destination, confidentiality, docStyle } = useDocument()
  ...
  const signature = useMemo(() => generateSignature(docId, name || 'EETRA', Date.now()), [docId, name])
  const qrUrl = useMemo(() => buildQrUrl(docId, signature, 88), [docId, signature])
  ...
  return (
    <div id="cover-page"
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 48, height: 48, background: co, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            {logoDataUrl
              ...
              : <div style={{ fontSize: 18, fontWeight: 900, color: '#fff', fontFamily: `'${fontTitle}', sans-serif`, letterSpacing: '-.5px' }}>{(name || 'EE').slice(0, 2)}</div>
            }
          </div>
          <div>
            <div style={{ fontFamily: `'${fontTitle}', sans-serif`, fontWeight: 800, fontSize: 15, color: '#111' }}>{name || 'EETRA'}</div>
      ...
      {/* Center content */}
      <div style={{ position: 'absolute', top: '43%', transform: 'translateY(-50%)', left: 64, right: 64 }}>
        ...
        <h1 style={{ fontFamily: `'${fontTitle}', sans-serif`, fontWeight: 900, fontSize: 62, letterSpacing: '-.03em', lineHeight: .92, textTransform: 'uppercase', color: '#111', marginBottom: 18 }}>
          {titleLine1}<br />{titleLine2}
        </h1>

        <p style={{ fontFamily: 'Libre Caslon Text, Georgia, serif', fontSize: 20, fontStyle: 'italic', fontWeight: 400, color: '#888', marginBottom: 32 }}>
      ...
      {/* QR code + Signature band (above footer) */}
      <div style={{ position: 'absolute', bottom: 72, left: 0, right: 0, padding: '10px 64px', borderTop: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: 16, background: '#fafafa' }}>
        <img
          src={qrUrl}
          alt="QR Vérification"
          style={{ width: 56, height: 56, borderRadius: 6, border: '1px solid #e8e8e8', flexShrink: 0 }}
          onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }}
        />
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
            <ShieldCheck size={11} color={co} />
            <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.12em', textTransform: 'uppercase', color: co }}>
              Authenticité EETRA Certifiée
            </span>
          </div>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 8, color: '#bbb', letterSpacing: '.04em', wordBreak: 'break-all' }}>
            {signature}
          </div>
          <div style={{ fontSize: 8, color: '#ccc', marginTop: 2 }}>
            Scannez le QR pour vérifier ce document sur eetra.app/verify
          </div>
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 72, background: co, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 64px' }}>
        <span style={{ fontFamily: `'${fontTitle}', sans-serif`, fontWeight: 800, fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,.75)' }}>{name || 'EETRA'}</span>
      ...
      <div style={{ position: 'absolute', right: 20, bottom: 80, fontFamily: 'Libre Caslon Text, Georgia, serif', fontSize: 32, fontStyle: 'italic', fontWeight: 300, color: '#ddd' }}>01</div>
    </div>
  )
}

...
interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> { variant?: 'primary' | 'ghost' | 'danger'; size?: 'sm' | 'md' | 'lg'; fullWidth?: boolean }
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', size = 'md', fullWidth, className, children, ...props }, ref) => {
    ...
    const sizes = { sm: 'px-3.5 py-2 text-[12px] tracking-wide', md: 'px-5 py-2.5 text-[13px] tracking-wide', lg: 'px-7 py-3.5 text-[14px] tracking-wide' }
    return <button ref={ref} className={cn(base, variants[variant], sizes[size], fullWidth && 'w-full justify-center', className)} {...props}>{children}</button>
  }
)

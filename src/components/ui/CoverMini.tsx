...
interface CoverMiniProps {
  ...
  /** Size variant — controls how detailed elements are rendered */
  size?: 'xs' | 'sm' | 'md' | 'lg'
}

/**
 * CoverMini — pixel-faithful A4 cover page rendered in HTML/CSS.
 *
 * Drop-in replacement for all SVG placeholder CoverMini usages.
 * Renders all 4 layouts (classic, bold, minimal, split) with:
 * - Company logo initial + name
 * - Document title
 * - Accent color system
 * - Date + QR placeholder
 * - Decorative geometry matching the real CoverPage component
 *
 * Usage:
 *   <div style={{ width: 80, aspectRatio: '0.707', overflow: 'hidden' }}>
 *     <CoverMini coverStyle={tpl.coverStyle} name={tpl.name} />
 *   </div>
 *
 * The component always fills 100%×100% of its container.
 * Wrap it in a container with the desired dimensions.
 */
export function CoverMini({
  ...
  // Title font size scaled by variant — mimics TITLE_SIZES from CoverPage.tsx
  ...
  if (layout === 'bold') return (
    <div style={{ ...base, background: accent, display: 'flex', flexDirection: 'column' }}>
      {/* Decorative orbs */}
  ...
  if (layout === 'minimal') return (
    <div style={{ ...base, background: '#fafbfc', display: 'flex', flexDirection: 'column' }}>
      {/* Bottom accent line */}

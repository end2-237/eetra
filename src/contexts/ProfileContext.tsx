...
export function ProfileProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try { const stored = localStorage.getItem(KEY); if (stored) setProfile({ ...DEFAULT_PROFILE, ...JSON.parse(stored) }) } catch {}
  }, [])
  const updateProfile = (u: Partial<CompanyProfile>) => {
    setProfile(p => { const next = { ...p, ...u }; try { localStorage.setItem(KEY, JSON.stringify(next)) } catch {}; return next })
  }
  const resetProfile = () => { setProfile(DEFAULT_PROFILE); try { localStorage.removeItem(KEY) } catch {} }
